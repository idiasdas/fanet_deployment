...
def test_movement_0() -> None:
    ...
    milp_model.cplex_model.solution.get_status() == OPTIMAL_SOLUTION
...
def test_movement_1() -> None:
    ...
    milp_model.cplex_model.solution.get_status() == INFEASIBLE_SOLUTION
    milp_model.cplex_finish()
...
def test_variables_types() -> None:
    ...
    targets_trace = TargetsTrace(n_targets=2, observation_period=observation_period)
    targets_trace.trace_set = [[(25,50),(25,50)],[(75,50),(75,50)]]
    graph = Graph(100, [10], (0,0,0), 1, 100, np.tan(np.pi/6))
    graph.deployment_positions = [(25,50,10),(75,50,10)]
    milp_model = MilpModel(n_available_drones=1,
    ...
    for t in range(observation_period):
        for p in graph.deployment_positions:
            assert milp_model.get_variable(milp_model.var_z_t_p(t,p))["type"] == BINARY_VARIABLE
    ...
    for t in range(observation_period):
        for drone in range(n_available_drones):
            assert milp_model.get_variable(milp_model.var_z_t_drone_p(t,drone,graph.base_station))["type"] == BINARY_VARIABLE
            for p in graph.deployment_positions:
                assert milp_model.get_variable(milp_model.var_z_t_drone_p(t,drone,p))["type"] == BINARY_VARIABLE
    ...
    for t in range(observation_period):
        for p in graph.deployment_positions + [graph.base_station]:
            for q in graph.get_positions_in_comm_range(p):
                assert milp_model.get_variable(milp_model.var_f_t_p_q(t,p,q))["type"] == CONTINUOUS_VARIABLE
    ...
    for t in range(observation_period):
        for sensor_trace in targets_trace.trace_set:
            ...
            for deployment_position in graph.get_target_coverage(sensor_position):
                assert milp_model.get_variable(milp_model.var_f_t_p_q(t,deployment_position,sensor_position))["type"] == CONTINUOUS_VARIABLE
            ...
            for drone in range(n_available_drones):
                for p in graph.deployment_positions + [graph.base_station]:
                    for q in graph.deployment_positions + [graph.base_station]:
                        assert milp_model.get_variable(milp_model.var_z_t_drone_p_q(t,drone,p,q))["type"] == BINARY_VARIABLE
